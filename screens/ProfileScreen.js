...
export default function ProfileScreen({ resetScrollTick, sessionUser }) {
    ...
    const fetchUserAndProfile = async () => {
        ...
        try {
            setUser(sessionUser);
            const authAvatar = sessionUser.user_metadata?.avatar_url || null;
            ...
            if (profileData) {
                ...
                if (!profileData.avatar_url && authAvatar) {
                    await supabase.from('profiles').update({ avatar_url: authAvatar }).eq('id', sessionUser.id);
                }
            } else {
