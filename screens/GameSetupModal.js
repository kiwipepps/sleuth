...
export default function GameSetupModal({ visible, onClose, onCreated, userId }) {
        ...
        try {
            ...
            if (isLocal) {
                ...
                if (participantEntries.length < 2) {
                    ...
                    setLoading(false);
                    return Alert.alert("Not Enough Agents", "Add at least 2 player names to start a local game.");
                }
                ...
                const assignmentPromises = participants.map(participant => {
                    const shuffled = shuffleArray(missionPool);
    ...
    const resetForm = () => {
        ...
        setMissionCount(3); setCalloutCount(2); setSelectedPack(null);
        onClose();
    };
